...
function refreshGameLayer(box, loop, offset) {
    ...
    for (let j = 0; j < box.children.length; j++) {
        ...
        rstyle.width = blockSize + 'px';
        rstyle.height = blockSize + 'px';
        r.className = r.className.replaceAll('bad', '');
...
function gameTapEvent(e) {
    ...
    if ((p.id == tar.id && tar.notEmpty) || (p.cell == 0 && x < blockSize) || (p.cell == 1 && x > blockSize && x < 2 *
        ...
        createjs.Sound.play("tap" + Math.floor(Math.random() * 2));
...
function shareText(score) {
    SubmitResults();
    if (score <= 30) return '小黑子树脂666';
    if (score <= 60) return '真正的ikun前来报到！';
    if (score <= 100) return '我喜欢唱 跳 rap 篮球！music！';
    if (score <= 150) return '黑粉头子是吧';
    return '你就是蔡徐坤本人？';
}
